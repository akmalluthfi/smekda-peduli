    ...
    <div class="row">
      <div class="col col-sm-10 col-md-8 col-lg-6">
        ...
        <div class="card">
          <div class="card-body">
            <h5 class="card-title">
              <a href="{{ route('campaigns.index') }}" class="link-secondary me-1"><i class="bi bi-arrow-left"></i></a>
              Buat Campaign Baru
            </h5>

            <form class="row g-3" method="POST" action="{{ route('campaigns.store') }}" enctype="multipart/form-data">
              @csrf
              <div class="col-12">
                <label for="title" class="form-label">Judul</label>
                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title"
                  name="title" value="{{ old('title') }}" placeholder="Masukkan judul campaign" required>
                @error('title')
              ...
              <div class="col-12">
                ...
                <input type="number" class="form-control @error('target_amount') is-invalid @enderror" id="target_amount"
                  name="target_amount" value="{{ old('target_amount') }}" placeholder="Masukkan jumlah target donasi"
                  required>
                @error('target_amount')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
